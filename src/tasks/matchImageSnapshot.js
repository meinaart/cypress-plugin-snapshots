...
async function matchImageSnapshot(data = {}) {
	...
	const actualFilename = getSnapshotFilename(
		testFile,
		snapshotTitle,
		IMAGE_TYPE_ACTUAL,
	)
	const diffFilename = getSnapshotFilename(
		testFile,
		snapshotTitle,
		IMAGE_TYPE_DIFF,
	)
	...
	const resized =
		options && options.resizeDevicePixelRatio && image.devicePixelRatio !== 1
	if (resized) {
	...
	const passed =
		expected && compareImages(expected, actual, diffFilename, options)
	...
	const diff =
		passed || autoPassed || !options.createDiffImage
			? undefined
			: createDiffObject(diffFilename)
