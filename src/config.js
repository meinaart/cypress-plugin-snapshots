...
function getImageConfig(options = {}) {
	return Object.keys(DEFAULT_IMAGE_CONFIG)
		.filter(
			(key) => options.imageConfig && options.imageConfig[key] !== undefined,
		)
		.reduce(
			(imageConfig, key) => {
...
function getScreenshotConfig(options = {}) {
	const screenshotConfig = Object.keys(DEFAULT_SCREENSHOT_CONFIG)
		.filter(
			(key) =>
				options.screenshotConfig && options.screenshotConfig[key] !== undefined,
		)
		.reduce(
			(currentConfig, key) => {
...
function shouldNormalize(dataType, suppliedConfig) {
	const cfg =
		suppliedConfig && suppliedConfig.normalizeJson !== undefined
			? suppliedConfig
			: getConfig()
...
function getPrettierConfig(dataType, suppliedConfig) {
	const cfg =
		suppliedConfig && suppliedConfig.prettierConfig
			? suppliedConfig
			: getConfig()
	return cfg.prettier && cfg.prettierConfig
		? cfg.prettierConfig[dataType]
		: undefined
}
