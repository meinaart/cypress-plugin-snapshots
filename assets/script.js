...
;(() => {
	...
	function readFile(
		fullPath,
		encoding = "base64",
		options = { log: false, timeout: 2000 },
	) {
	...
	function wrapImage(type, title, dataUri = "", imageStyle = "") {
		...
		return (
			...
			wrapImageStyle(
				imageStyle,
				dataUri
					? `<img src="${dataUri}" alt="${title}" class="snapshot-image__image">`
					: "",
			) +
			"</div>"
		)
	...
	function formatImageResult(data) {
		...
		return Cypress.Promise.all(promises).then(
			([expectedImage, actualImage, diffImage = ""]) => {
	...
	function formatResult(data) {
		...
		return Cypress.Promise.resolve(
			formatPreview(title, htmlEncode(data.actual)),
		)
	}

	Cypress.$(document.body).on(
		"click",
		...
		(e) => {
			...
			if (data) {
				formatResult(data).then((diffHtml) => {
					...
					Cypress.$(".snapshot-btn-close", top.document).on(
						"click",
						closeSnapshotModal,
					)

					Cypress.$(".snapshot-btn-approve", top.document).on("click", () => {
